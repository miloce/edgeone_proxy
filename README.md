...
- **无服务器化部署**：基于 EdgeOne Edge Functions，无需购买和管理服务器。
- **会话保持**：向客户端返回上游 Cookie 的原始作用域元数据，支持多子域会话。
...
2.  **`edge-functions/proxy.js`**
    ...
    - **行为**：代理请求和响应、改写重定向，并通过 `X-Proxy-Set-Cookie` 返回上游 Cookie 的原始 Domain/Path 信息。
